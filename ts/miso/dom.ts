...
export function diff(currentObj: VTree, newObj: VTree, parent: Element, context: Context): void {
  ...
  else diffNodes(currentObj, newObj, parent, context);
}
...
function replace(c: VTree, n: VTree, parent: Element, context : Context): void {
  ...
  if (n['type'] === 'vtext') {
    ...
    createElement(n, parent, context, (newChild: Element) => {
...
export function populate(c: VTree, n: VTree, context: Context): void {
  if (n['type'] !== 'vtext') {
    if (!c) {
        if (n['type'] === 'vfragment') c = vfragment({});
        else c = vnode({});
    }
    ...
    if (n['type'] === 'vnode') {
      ...
      diffChildren(c as VFragment, n as VFragment, n['domRef'], context);
    }
    drawCanvas(n);
  }
}
...
function diffChildren(c: VNode, n: VNode, parent: Element, context: Context): void {
  if (c['shouldSync'] && n['shouldSync']) {
    ...
    const longest: number = Math.max(c.children.length, n.children.length);
    for (let i = 0; i < longest; i++)
      diff(c.children[i], n.children[i], parent, context);
  }
}

function populateDomRef(obj: VTree, parent: Element, context: Context): void {
  if (obj['ns'] === 'svg') {
...
function createElement(obj: VTree, parent: Element, context: Context, attach: (e: Node) => void): void {
...
function create(obj: VTree, parent: Element, context: Context): void {
  if (obj['type'] === 'vtext') {
    ...
    createElement(obj, parent, context, (child: Element) => {
      ...
      */
      if (found) {
        ...
        -> [ b e a j   ] <- new children
             ^
        */ else {
        createElement(nFirst, parent, context, (e: Element) => {
